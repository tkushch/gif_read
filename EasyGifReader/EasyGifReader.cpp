...
struct EasyGifReader::Internal {
    ...
    static void copyRows(uint32_t *dst, const uint32_t *src, int width, int height, size_t dstStride,
                         size_t srcStride);
    ...
    static bool readLoopExtension(int &loopCount, const ExtensionBlock *extensionBlocks,
...
static int openFileHandleUTF8Win(const char *utf8Filename, int oFlag, int pMode) {
    ...
    int wideFilenameLen =
        MultiByteToWideChar(CP_UTF8, 0, utf8Filename, utf8FilenameLen, wideFilename, wideFilenameCapacity);
    ...
    delete[] wideFilename;
    if (fd == -1) throw EasyGifReader::Error::OPEN_FAILED;
    return fd;
}
#endif
...
EasyGifReader::FrameBounds EasyGifReader::Internal::frameBounds(GifFileType *gif, int imageIndex) {
    ...
    return FrameBounds{std::max(imageDesc.Left, 0), std::max(imageDesc.Top, 0),
...
void EasyGifReader::Internal::copyRows(uint32_t *dst, const uint32_t *src, int width, int height,
...
int EasyGifReader::FrameDuration::milliseconds() const { return 10 * centiseconds; }

double EasyGifReader::FrameDuration::seconds() const { return .01 * centiseconds; }
...
EasyGifReader::Frame::~Frame() { delete[] pixelBuffer; }

EasyGifReader::Frame &EasyGifReader::Frame::operator=(const Frame &orig) {
    if (this != &orig) {
        bool keepPixelBuffer = pixelBuffer && orig.pixelBuffer &&
                               parentData->pixelBufferSize == orig.parentData->pixelBufferSize;
        if (!keepPixelBuffer) {
            delete[] pixelBuffer;
            pixelBuffer = nullptr;
        }
        ...
        delay = orig.delay;
        if (orig.pixelBuffer) {
            if (!keepPixelBuffer) pixelBuffer = new uint32_t[parentData->pixelBufferSize];
            memcpy(pixelBuffer, orig.pixelBuffer, sizeof(uint32_t) * parentData->pixelBufferSize);
        }
    }
    return *this;
...
const uint32_t *EasyGifReader::Frame::pixels() const { return pixelBuffer; }

int EasyGifReader::Frame::width() const { return w; }

int EasyGifReader::Frame::height() const { return h; }

EasyGifReader::FrameDuration EasyGifReader::Frame::duration() const {
    return FrameDuration{delay > 1 ? delay : 10};
}

EasyGifReader::FrameDuration EasyGifReader::Frame::rawDuration() const { return FrameDuration{delay}; }

void EasyGifReader::Frame::nextFrame() {
    if (!(parentData && parentData->gif)) throw Error::INVALID_OPERATION;
    ++index;
    if (!parentData->gif->ImageCount ||
        (parentData->loopCount && index >= parentData->loopCount * parentData->gif->ImageCount))
        return;
    ...
    if (!colorMap) colorMap = parentData->gif->SColorMap;
    ...
    if (!colorMap || (!raster && imageDesc.Width > 0 && imageDesc.Height > 0)) throw Error::INVALID_GIF_FILE;
    FrameBounds frameBounds = Internal::frameBounds(parentData->gif, imageIndex);
    if (gcb.TransparentColor >= 0 || frameBounds.x0 > 0 || frameBounds.x1 < w || frameBounds.y0 > 0 ||
        frameBounds.y1 < h || gcb.DisposalMode == DISPOSE_PREVIOUS) {
        if (!imageIndex)
            ...
            if (disposal == DISPOSE_PREVIOUS)
                Internal::copyRows(corner(prevBounds), pixelBuffer + w * h, prevBounds.width(),
                                   prevBounds.height(), w, prevBounds.width());
            else
    ...
    if (disposal == DISPOSE_PREVIOUS) {
        if (imageIndex && imageIndex < parentData->gif->ImageCount - 1)
            Internal::copyRows(pixelBuffer + w * h, corner(frameBounds), frameBounds.width(),
                               frameBounds.height(), frameBounds.width(), w);
        else
    ...
    for (int y = frameBounds.y0; y < frameBounds.y1; ++y) {
        ...
        const GifByteType *src =
            raster + (imageDesc.Width * (y - imageDesc.Top) + (frameBounds.x0 - imageDesc.Left));
...
EasyGifReader::FrameIterator::FrameIterator(const EasyGifReader *decoder, Position position) {
    if (decoder) {
        if (!((parentData = decoder->data) && parentData->gif)) throw Error::INVALID_OPERATION;
...
EasyGifReader EasyGifReader::openCustom(size_t (*readFunction)(void *outData, size_t size, void *userPtr),
                                        void *userPtr) {
    Internal *data = nullptr;
    try {
        data = new Internal;
...
int EasyGifReader::FrameBounds::width() const { return x1 - x0; }

int EasyGifReader::FrameBounds::height() const { return y1 - y0; }

bool EasyGifReader::Internal::readLoopExtension(int &loopCount, const ExtensionBlock *extensionBlocks,
                                                int extensionBlockCount) {
    for (; --extensionBlockCount > 0; ++extensionBlocks) {
        if (extensionBlocks[0].Function == 0xff && extensionBlocks[0].ByteCount == 11 &&
            (!memcmp(extensionBlocks[0].Bytes, "NETSCAPE2.0", 11) ||
             !memcmp(extensionBlocks[0].Bytes, "ANIMEXTS1.0", 11)) &&
            extensionBlocks[1].Function == 0x00 && extensionBlocks[1].ByteCount == 3 &&
...
EasyGifReader::EasyGifReader(Internal *data) : data(data) {
    ...
    data->loopCount = 1;
    if (!Internal::readLoopExtension(data->loopCount, data->gif->ExtensionBlocks,
                                     data->gif->ExtensionBlockCount) &&
        data->gif->ImageCount > 0)
        Internal::readLoopExtension(data->loopCount, data->gif->SavedImages[0].ExtensionBlocks,
                                    data->gif->SavedImages[0].ExtensionBlockCount);
    ...
    for (int i = 0; i < data->gif->ImageCount - 1; ++i) {
        ...
        if (gcb.DisposalMode == DISPOSE_PREVIOUS) {
            size_t area = (size_t)data->gif->SavedImages[i].ImageDesc.Width *
                          (size_t)data->gif->SavedImages[i].ImageDesc.Height;
            if (area > prevFrameBufferSize) {
                prevFrameBufferSize = area;
                if (area == frameArea) break;
            }
        }
    }
...
EasyGifReader::EasyGifReader(EasyGifReader &&orig) : data(orig.data) { orig.data = nullptr; }
...
int EasyGifReader::width() const { return data->gif->SWidth; }

int EasyGifReader::height() const { return data->gif->SHeight; }

int EasyGifReader::frameCount() const { return data->gif->ImageCount; }

int EasyGifReader::repeatCount() const { return data->loopCount; }

bool EasyGifReader::repeatInfinitely() const { return !data->loopCount; }

EasyGifReader::FrameIterator EasyGifReader::begin() const {
    return FrameIterator(this, FrameIterator::BEGIN);
}

EasyGifReader::FrameIterator EasyGifReader::end() const { return FrameIterator(this, FrameIterator::END); }
